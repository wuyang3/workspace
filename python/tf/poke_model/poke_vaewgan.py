...
class PokeVAEWGAN(ConvAE):
    ...
    def __init__(self, learning_rate=1e-4, episilon=1e-7, batch_size=16,
        ...
        with tf.variable_scope('generator', reuse=True):
            z_uniform = self.random_sample(batch_size, 1024)
        ...
        self.loss = self.loss_function(activation_real, activation_rec,
                                       ...
                                       d_real, d_rec, d_fake)
        self.train_op_e, self.train_op_g, self.train_op_d, self.clip_d = self.train(
    ...
    def encode(self, img):
        with tf.variable_scope('encoder'):
            ...
            #fc6 = self.fc_bn_layer(conv5_flat, 2048, is_training=self.is_training,
            #                       initializer_type=1, name='fc6')
            fc6 = self.fc_no_activation(conv5_flat, 2048, initializer_type=1, name='fc6')
        return fc6, shape
    ...
    def generate(self, code, shape):
        ...
        deconv3 = self.deconv_bn_layer_with_stride(deconv4, [5, 5], [128, 64], 2,
                                                   ...
                                                   initializer_type=1,
                                                   name='deconv3')
        deconv2 = self.deconv_bn_layer_with_stride(deconv3, [5, 5], [64, 32], 2,
                                                   is_training=self.is_training,
                                                   initializer_type=1,
                                                   name='deconv2')
    ...
    def discriminate(self, img):
        ...
        fc0 = self.fc_layer(conv2_flat, 512,
    ...
    def loss_function(self, act_real, act_rec,
        ...
        with tf.variable_scope('loss'):
            ...
            # inverse loss of D measures the wasserstein distance between two distributions.
            # D_real - D_fake: Wasserstein distance. The smaller, the better GAN is trained.
            # if you want to minimize the distance of both encoded and prior distribution,
            # they should both be subtracted D_real. Otherwise the loss would blow up.
            with tf.name_scope('D'):
                loss_f['D'] = tf.reduce_mean(0.5*(D_rec + D_fake) - D_real)
    ...
    def train(self, loss, learning_rate, epsilon):
        ...
        with tf.variable_scope('train'):
            ...
            clip_D = [tf.assign(var, tf.clip_by_value(var, self.clamp_lower, self.clamp_upper))
                      for var in d_vars]
        return train_op_e, train_op_g, train_op_d, clip_D
